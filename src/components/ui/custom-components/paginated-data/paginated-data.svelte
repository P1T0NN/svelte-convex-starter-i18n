...
<Pagination count={Number.MAX_SAFE_INTEGER} class={className} aria-busy={loading}>
	<PaginationContent class="w-full">
		<span class="mr-auto text-sm text-muted-foreground">
			{totalPages != null
				? m['Components.PaginatedData.pageWithTotal']({ page, total: totalPages })
				: m['Components.PaginatedData.page']({ page })}
		</span>
		<PaginationItem>
			<Button
				variant="outline"
				...
				onclick={onPrev}
				aria-label={m['Components.PaginatedData.previousPage']()}
			>
		...
		<PaginationItem>
			<Button
				...
				onclick={onNext}
				aria-label={m['Components.PaginatedData.nextPage']()}
			>
